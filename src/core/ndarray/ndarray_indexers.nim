...
proc get_indexer(idx: NdArray[int]): Indexer=
  assert(idx.shape.len == 1)
  result = new Indexer
  result.idx_seq = idx.toSeq
  result.idx_type = INDEX_TYPE.ARRAY
...
proc get_all_coordinates*(indexer_list: seq[Indexer]): seq[seq[int]]=
  ...
  if check_all_int_indexer(indexer_list):
    ...
    for axis in 0 ..< indexer_list.len:
      if indexer_list[axis].idx_type == INDEX_TYPE.ARRAY:
        result.add(indexer_list[axis].idx_seq)
      else:
        result.add(xrange(indexer_list[axis].lowIdx, indexer_list[axis].highIdx, 1, inclusive=true))
    if indexer_list.len == 1:
      var
        tmp :seq[seq[int]] = @[]
      ...
      for axis in 0 ..< indexer_list.len:
        if indexer_list[axis].idx_type == INDEX_TYPE.ARRAY:
          skip_axis.add(axis)
      result.sort((l1,l2) => cmp_seq_lexical_skip_axis(l1,l2,skip_axis) )

proc get_memory_blocks_along_axis*(shape: seq[int], axis: int): (seq[seq[int]], int)=
  var
    fake_list_indexer = get_indexer_list(shape)
    block_length = fake_list_indexer[axis].highIdx - fake_list_indexer[axis].lowIdx + 1
  fake_list_indexer[axis].highIdx = fake_list_indexer[axis].lowIdx
  var
    coordindates = fake_list_indexer.get_all_coordinates()
  result = (coordindates, block_length)
