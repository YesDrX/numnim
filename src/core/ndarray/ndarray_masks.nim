...
proc get_bool_mask_float[T:SomeFloat](input: NdArray[T], value:T, op: string): NdArray[bool]=
  ...
  if op == "isnan":
    for i, coordinate in coordindates:
      data_buffer[i] = input.data_buffer[get_location_from_coordinate(input.start_idx_in_buffer, input.strides, coordinate)].classify == fcNan
  elif op == "isinf":
    for i, coordinate in coordindates:
      data_buffer[i] = input.data_buffer[get_location_from_coordinate(input.start_idx_in_buffer, input.strides, coordinate)] ==  Inf
  elif op == "isfinite":
    for i, coordinate in coordindates:
      data_buffer[i] = input.data_buffer[get_location_from_coordinate(input.start_idx_in_buffer, input.strides, coordinate)].classify != fcNan and input.data_buffer[get_location_from_coordinate(input.start_idx_in_buffer, input.strides, coordinate)] != Inf
  result = data_buffer.toNdArray(input.shape)
...
proc `[]=`*[T](input: NdArray[T], mask: NdArray[bool], another_array: NdArray[T]) =
  var
    coordinates = get_coordinates_from_mask(mask)
...
when isMainModule:
  var
    a = @[1.0,2.0,3.0,NaN].toNdArray(@[2,2])
  # echo a == b
  echo a
  echo (a > 2)
  echo (~(a > 2)) & (a<2)
