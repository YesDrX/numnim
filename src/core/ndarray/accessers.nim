...
proc assign_view*[T](base_array, another_array: NdArray[T], indexer_list: seq[Indexer])=
  ...
  var
    toassign = base_array.slice_view(start_coordinate_offset, sliced_shape)
  ...
  left_memory_stride = toassign.strides[sliced_view_memory_blocks_axis]
  ...
  for i, coordinate in sliced_view_memory_block_new_coordinates:
    left_memory_block_offset = get_location_from_coordinate(toassign.start_idx_in_buffer, toassign.strides, coordinate)
    ...
    left_vector = ndarray_memory_block_to_blas_vector(toassign, left_memory_block_offset, left_memory_stride, sliced_view_memory_blocks_axis_len)
...
proc assign_non_view*[T](base_array, another_array: NdArray[T], indexer_list: seq[Indexer])=
...
proc assign_sliced*[T](base_array, another_array: NdArray[T], indexer_list: seq[Indexer])=
  ...
  if indexer_list.check_no_array_indexer:
    assign_view[T](base_array, another_array, indexer_list)
  else:
    assign_non_view[T](base_array, another_array, indexer_list)
...
proc `[]=`*[T;U0](input: NdArray[T], idx0: U0, another_array: NdArray[T]): void=
  var
    indexer_list = get_indexer_list(input.shape, idx0)
  assign_sliced(input, another_array, indexer_list)
