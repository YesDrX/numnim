...
proc cmp_seq_lexical_skip_axis*[T](list1: seq[T], list2: seq[T], skip_axis:seq[int]): int=
  # test if list1 < list2
  assert(list1.len == list2.len and list1.low == list2.low and list1.high == list2.high, fmt"to compare two sequence lists, their length must be equal.")
  result = 0
  for idx in list1.low .. list1.high:
    if idx in skip_axis: continue
    if list1[idx] < list2[idx]:
      return -1
    elif list1[idx] > list2[idx]:
      return 1
...
proc fixedWidthStr*(input: string, width = 10, align = "left", fill = ' ') :string=
  result = input
  if result.len > width:
    result = result[0 .. 9]
  else:
    var
      extra_spaces = width - input.len
    if align.toLowerAscii == "left":
      result &= fill.repeat(extra_spaces)
    elif align.toLowerAscii == "right":
      result = fill.repeat(extra_spaces) & result
    elif align.toLowerAscii == "center":
      var
        pre = extra_spaces div 2
        post = extra_spaces div 2 + extra_spaces mod 2
      result = fill.repeat(pre) & result & fill.repeat(post)
    else:
      raise newException(Exception, fmt"Unknown align parameter {align}. Acceptable ones are : left/right/center.")
export
  strformat,
  sequtils,
  strutils
...
when isMainModule:
  ...
  echo c
  # echo string(1)
  echo "abc".fixedWidthStr(20, align="right", fill='_')